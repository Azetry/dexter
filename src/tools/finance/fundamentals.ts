...
export const getIncomeStatements = new DynamicStructuredTool({
  ...
  func: async (input) => {
    ...
    const { data, url } = await callApi('/financials/income-statements/', params, { cacheable: true });
...
export const getBalanceSheets = new DynamicStructuredTool({
  ...
  func: async (input) => {
    const params = createParams(input);
    const { data, url } = await callApi('/financials/balance-sheets/', params, { cacheable: true });
...
export const getCashFlowStatements = new DynamicStructuredTool({
  ...
  func: async (input) => {
    const params = createParams(input);
    const { data, url } = await callApi('/financials/cash-flow-statements/', params, { cacheable: true });
...
export const getAllFinancialStatements = new DynamicStructuredTool({
  ...
  func: async (input) => {
    const params = createParams(input);
    const { data, url } = await callApi('/financials/', params, { cacheable: true });
